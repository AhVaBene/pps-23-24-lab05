...
object Item:
  def apply(code: Int, name: String, tags: String*): Item =
    var tagsSequence = Sequence[String]()
    tags.foreach(t =>
      tagsSequence = tagsSequence.concat(Sequence(t)))
...
object Warehouse:
  ...
  private case class WarehouseImpl() extends Warehouse:
    private var items = Sequence[Item]()
    override def store(item: Item): Unit = items = items.concat(Sequence(item))
    override def searchItems(tag: String): Sequence[Item] = items.filter(i => i.tags.contains(tag))
    override def retrieve(code: Int): Optional[Item] = items.find(i => i.code == code)
    override def remove(item: Item): Unit = items = items.filter(i => i != item)
    override def contains(itemCode: Int): Boolean = !items.find(i => i.code == itemCode).isEmpty
